            ...
            @endphp
              ...
              <div class="chat_list user-select chat-count {{$chat->job_id}}-{{$chat->applicant->submit_by}}" data-job_id="{{$chat->job_id}}" data-applicant_id="{{$chat->applicant->submit_by}}">
                <div class="chat_people">
                  <div class="chat_img"> <img src="{{isset($chat->applicant->proposal_to_user->logo) ? asset($chat->applicant->proposal_to_user->logo) : asset('destination/images/aplicant.png')}}" alt="sunil"> </div>
                  <div class="chat_ib">
                    <h5>{{$chat->applicant->proposal_to_user->first_name}} <span class="chat_date">{{$chat->created_at->format('M d')}}</span><span class="count">{{$count}}</span></h5>
